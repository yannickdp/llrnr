...
2. **Celestial (day only here):** sun + glow, clouds, birds. (Moon & stars are
   drawn in the night pass so they stay bright above the dark tint.)
...
7. **Night pass (if night):** dark tint overlay → bloom every `glowTarget` and
   every `emitter` → draw moon + twinkling stars.
...
function nightPass(t) {
  ctx.fillStyle = "rgba(12,16,44,0.55)";                 // dim everything
  ...
  drawMoonStars(t);                                       // moon bloom + twinkle
}
```
